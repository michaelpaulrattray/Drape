...
**Implementation status:** C1, C2, H1, H6, M7 option 1, and M10 are implemented and committed on local `main`. The C1 follow-through also closes the remaining known guard-dependent Canvas writes and scopes Wardrobe session access at the durable statement; R7's outward account and Cast reads now use positive response projections; and new public Cast identifiers use cryptographic 80-bit Klieg Identity codes with bounded unique-index collision retry. These commits have not been pushed or deployed by this workstream; production status must be verified separately before any finding is called closed in production. All remaining findings must be resolved or explicitly accepted before the invite gate opens.
...
| Additional hardening — public Cast identifiers | `8b3ecd5` — new mints receive cryptographically random 80-bit `KI-XXXX-XXXX-XXXX-XXXX` identifiers; legacy `MOD-*` identifiers remain valid; exact unique-index collisions retry only the atomic settlement with no repeated generation or charge | Not deployed; disposable-MySQL collision, rollback, and no-residue proof passed on 2026-07-25 |
...
**The H2/H3/H4 re-ask this finding triggered has now been done (2026-08-19), and it changes two of the three.** H2 (`isIpBlocked`) was **never wired** — confirmed at birth in `098de49d`, where the only non-test caller was `blockIp` checking for a duplicate, and `RATE_LIMITING.md` described the request-path check in the same commit that failed to write it. H3 (the admin allowlist) is **invoked on every admin request** through `validateAdminAccess` in `adminProcedure`; it is inert because the list is empty, not because nothing calls it — this audit's framing of it as an uninvoked control is wrong and CLAUDE.md now says so. H4 is the same road as H5: `isSensitiveAction` had a live call site in `server/routers.ts` and was dropped by `3cb0cdee`, the commit that split that file into 17 modules — **six hours before `41a765ea` did the same thing to these caps, on the same day.**
