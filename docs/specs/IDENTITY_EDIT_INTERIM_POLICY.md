...
//     hardened editor; R1b below-shoulder length REFUSED), but every entry stays
...
### 8.3 Face/hair/skin identity leaves — drafts only, anchor view only (RATIFIED: R1 allow, R1b refuse, R1c allow text-only)
...
**Below-shoulder hair length refuses during R6 (ratified R1b)** — the bare-shoulders headshot (`geminiGeneration.ts:565`) cannot reliably evidence it; above-shoulder hair edits stay eligible under R1. Recorded as a **reconsideration candidate** for Batch D/R7 once full-body evidence, revision-wide regeneration, and calibration exist — not promised merely because R7 begins.
...
| `person.hair.length` | allow above shoulders; **below-shoulder refuses (R1b ratified)** | `hairLength` | prompt+pref only | length within style transfer | length descriptor | below-shoulder: not anchor-evidenced — refused in R6; Batch D/R7 reconsideration candidate, not promised |
...
3. **Tier-view validity.** Every selected-tier view must be filled, fresh (not stale — pinned or unpinned), successful (no failed marker), and belong to the current identity revision. Cross-revision tier views refuse with the §7.4 copy.
...
Typed, honest, never advertising unsupported capability, temporary limits framed as temporary. In addition to rev-4 copy (marks "not yet" with ink-only brief pointer per R6; presentation routing + creation-correction; build/age/gender/skin-tone re-cast routing; F4 on minted; non-anchor redirect; R1b length; vague reference; unsupported reference subtype; creation reference; classifier outage; mint integrity; F5 placeholder and the UI promise sweep — `MasterPromptPanel.tsx:254/302`, `LoadingOverlay.tsx:22/47/53`, `RefinePanel.tsx:11`):
...
|---|---|---|
| M1 | iterate — identity ⊕ | Mark families × operations, text AND reference, refuse before deduction. §8.2 categories refuse. Ratified R1 leaves proceed on draft+`frontClose` only: `anchor` role, new revision, ALL heads flagged incl. pinned; non-anchor redirect; minted F4; R1b terms refuse; parent-only requests refuse as ambiguous; unmapped leaves (chin, browColor) refuse. Mixed/multi-leaf most-restrictive-wins: one refused leaf refuses the whole request. |
...
| M7 | mint integrity ⊕ | Guard regressions. §14's three checks, each with its own copy: (1) same-revision `display` headshot over an older authoritative anchor **passes**; (2) mint-time slot generation still consumes the anchor (not the display asset) via the §7 selector; (3) a stale or failed anchor refuses; (4) a displayed headshot from an earlier or unknown revision refuses; (5) current-revision tier views pass; (6) cross-revision tier views refuse; (7) the legacy fingerprint-match case passes AND the fingerprint-mismatch case refuses — both explicit. Stale tier views refuse pinned and unpinned; failed/missing slot refuses; dialog predicts per check; credits per M20. |
...
| **R1b** | Below-shoulder hair length | **REFUSE during R6** — the headshot cannot reliably evidence it; above-shoulder hair edits stay eligible under R1. Recorded as a reconsideration candidate for Batch D/R7 once full-body evidence, revision-wide regeneration, and calibration exist — **not** promised merely because R7 begins. |
...
| **R8** | Mint integrity | **ENFORCE the three separate validity checks** (§14): identity-anchor validity, display-headshot validity, selected-tier view validity. Refuse stale, failed, missing, cross-revision, and unknown-authority states with state-specific, graceful copy that says what is wrong and how to resolve it — never one vague "headshot out of sync" message. |
