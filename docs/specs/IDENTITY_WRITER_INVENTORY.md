...
| B5 | `models.delete` | `routes/models.ts` → `db/models.ts deleteModel` | **DRAFTS-ONLY (founder-ruled, final round, tested):** minted/locked → PRECONDITION_FAILED with honest copy; archived → NOT_FOUND (FR-4); foreign → FORBIDDEN; `deleteModel` result checked — a db failure is an error, never success; audit log fires only after a successful delete. UI affordances reconciled: the lobby Recent-Work card disables delete on minted casting rows with honest copy (the feed now carries `draft: boolean` server-side); `WardrobeStart`'s delete affordance is removed (its gallery is minted-only). Drafts hard-delete as before. **A hard-deleted source row still leaves dangling historical placements**: `board_items.sourceModelId` (and D-12 snapshots / `generations.modelId`) point at a row that no longer exists — distinct from the archived-source degrade, which requires a live row to detect, so a deleted-source placement does NOT currently render "Source unavailable"; its stored image renders from the snapshot and op paths 404. **Full missing-source rendering / cascade / R2 cleanup / archive design remains the explicit R7 item.** |
...
- **Archived board placements (review item 2, tested server + browser):** `boards.getItems` stamps `sourceArchived: true` on items whose source model is archived (batched lookup, `getArchivedModelIdsIn`); the flag survives BoardPage's canvasItems rebuild and the CastNode face renders the D-12 **"Source unavailable"** state (browser leg UI1: fallback text present, stored image not rendered as live) — stored snapshot data (label/imageUrl/metadata) is retained on the row as historical evidence. `boards.getItemModelInfo` returns `model: null` + `sourceArchived: true` (never conflated with an ordinary unlinked item — NodeInfoPanel says so explicitly), zero asset-ledger exposure. Edit and package observers are disabled on archived-source nodes.
