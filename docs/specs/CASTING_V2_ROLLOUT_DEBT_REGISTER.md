...
|---|---|---|---|
| `CASTING_INK_STUDIO_SCOPE` | ⚠ **RETIRED, and the tripwire is MOOT rather than satisfied** (#1158, his *"It retires with N2"*). It read: does not pass `users:1` while uploads ride uncropped to the plate mint, re-arming the moment the mannequin road resumes. The mint went in slice 2, the plate carry in slice 4f, and `MANNEQUIN_ROAD_DEFERRED` was deleted with its last gate — so there is no road to resume and no upload door to widen | 2026-09-24 | **RETIRED** — the flag is unset on the service |
| `CASTING_INK_CUT_SCOPE` | ✅ **RETIRED AND UNSET ON THE SERVICE 2026-09-24** — his *"It retires with N2"* (#1158 slice 4c) took the whole studio road, and **#10 / 3a.2(b) retired with it**: its subject is a preview on an upload surface that no longer exists. This row is kept as the record, and the debt is CLOSED rather than outstanding. ~~**3a.2(b), the customer-facing preview that shows her the cut.** His own narrowing (*"his account only until the customer-facing preview ships"*, fable-1257 §1)~~ | 2026-08-23, flipped on his word; UNSET 2026-09-24 | ~~**FINISH** — one named surface~~ — **CLOSED with #10** |
