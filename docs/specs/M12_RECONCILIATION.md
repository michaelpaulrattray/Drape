...
## The founder question, reframed — ✅ **ANSWERED 2026-08-17. Identity LOCKS at Sign (D-247).**

> **His words, in the sitting (fable-840 §B.4):** *"the identity is locked but
> you can maybe do different takes e.g she is wearing this dress or in this
> envrioment or wearing these earrings etc etc not sure have not figured out
> the best usecase for takes yet. but identity should be locked."*
>
> **So the first of the three answers below is the ruling**, and nothing here
> is a recommendation any more. `already_signed` is the product's answer.
> **Rows 4 and 5 unblock on it**: there is no revision to route and no affected
> view to derive, because nothing changes a signed identity — and row 11 stays
> superseded for the same reason.
>
> **What did NOT get decided, and must not be read into it:** *Takes*. He named
> the idea and said he has not figured out the use case, which under the
> confirmed-before-filed rule is an idea on the record rather than a decision.
> **No Take is built without a fresh founder yes**; M8 is where it returns.
>
> **One rider, owed and small:** `CastingRoom.tsx:481` still reads *"Refining a
> signed Cast arrives with refinement."* The surface it sits on is already the
> Takes section under a disabled "New takes" button, so only the verb is wrong —
> identity edits are not arriving. It is a one-word re-word riding the next
> commit into that room, and it is written down here so it has an owner rather
> than a "when next touched".

The section below is kept as the record of how the question was priced, and
because the three answers are what make the ruling's cost legible. The first
pass asked *"does a Cast have one view or several?"* That question is
