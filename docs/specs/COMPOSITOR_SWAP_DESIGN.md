...
| **img-left / img-right asymmetry** | **NARROWED 2026-08-17 — the image-side axis is ANSWERED, by a court convened for a different feature** (opus-603, ruled fable-814 §3). The row's original observation (ten for ten on GPT2 the smaller, more occluded hoop scores worse) confounds **size, occlusion and image side**. `V4_SIDE_INFERENCE_COURT.md` settles the third: per-instance asks scored **her right 3/6 · her left 6/6**, and its own sentence is *"the misses all landed on the image's RIGHT half whatever the recipe named"* — *"the engine appears to favour the image's right half regardless of which side the recipe named."* Same engine family by construction (the court ran on the repaint road, which dispatches `repaintEngine ?? defaultMaskedEditEngine` → GPT Image 2, and `CASTING_SIDE_PHRASING_SCOPE` cannot be set outside `CASTING_REPAINT_SCOPE`). **What REMAINS open is the size/occlusion half only.** *Not offered as the lean, and stated so it is not re-offered:* the edit-law cell's row d (opus-134, "one ear edited, other held — NOT HELD", held ear her right) is n=2 and confounded with the carry defect the cell was measuring — its own conclusion is about references, not sides. | **No mechanism test is built** — the court bought the answer on a better subject (per-eye, n=12 across two phrasings, mirrored arm), and the standing rule already contains the risk in production: **per-instance worst-of-n, never averaged.** The narrowed remainder keeps a trigger: **a hoop case the per-instance rule fails to contain, or the size/occlusion question opening on its own evidence.** |
...
| **`applied` is stored nowhere** | **NAMED GAP, 2026-08-11 (fable-224).** The cut is `applied ∩ (delivered ∪ master)` and `applied` is `min(feather(zone), edgeMatte)`, computed in the compositor and discarded. So **a cut that loses ground cannot be diagnosed from any artifact we keep** — the painted frame's lesson (founder-queue item 1) one layer over. Found when an attribution-by-elimination failed its own control (20/15 px the crop could not own) and had to be withdrawn. | **COSTED 2026-08-11, and it splits in two.** The capture mechanism already expects it — `DiagnosticFrame`'s own doc names `painted`, `composite`, **`applied`** — and `composed.applied` is in scope at `maskedRefine`'s capture site, which passes the first two and not the third. So the cheap half is **one line plus a `writeMaskPng` helper**: no migration, no flag, no bucket, riding the existing `CASTING_DIAGNOSTIC_CAPTURE_SCOPE`, cleanup reservation and purge promise. **But that site fires only on `composite_fault` REFUSALS, and the attribution that had to be withdrawn was a DELIVERED render** — so the free half would not have answered the question that named this gap. Capturing `applied` on delivered renders is outside the founder's narrow 2026-08-08 approval (*refused and faulted renders only*) and is a founder line. ✅ **THE CHEAP HALF LANDED (read at the code 2026-08-24).** `maskedRefine.ts` imports `writeMaskPng` and emits `{ name: "applied", bytes: await writeMaskPng(composed.applied) }` beside `painted` and `composite`, under a comment quoting fable-233 §3 and restating this row's own founder half in its own terms: *"this site fires only on `composite_fault` — capturing `applied` on delivery is outside the founder's narrow approval and is his line, not ours."* So the row is stale in its BUILT half and exactly right in its FOUNDER half, which is why the clause is kept rather than struck. What is still owed is unchanged: `applied` on a DELIVERED render, and it is his line. |
| ~~**The read-back caption is stored nowhere**~~ | **WITHDRAWN 2026-08-11 — the premise was false, and the artifact says so.** Captions ARE persisted: `landVariant` writes `internalPrompt.captions` on the variant row, before the mint runs. So the elimination that produced this row was unnecessary, and its conclusion is now a READING: v#144 carries a `statedAccessories` caption and minted lib#10/#11; **v#145 and v#146 carry `hairWorn` and no `statedAccessories` at all**, and minted nothing. All five hoop renders wrote the facet (segments 5–9). Nothing to build; this row was going to be quoted as a reason to build it. | — |
| **A paid render can contribute nothing to the library and say nothing about it** | **OPEN, and much smaller than the row above claimed.** The diagnosis takes one query rather than an hour, but no line announces the outcome: when `slots.length === 0` the only log is `unfiled` at `info`. ~8 lines and a driven test; no migration, no flag. ⚠ **STILL OPEN, driven at the code 2026-08-24 rather than assumed** — and the reading changes the row's SIZE without changing its verdict. `refineService` now logs `unfiled` at `info` carrying a per-facet REASON over a named `UnfiledReason` vocabulary (*"a facet this render wrote had no library slot to file in"*), plus a SECOND line for open kinds (*"an open kind this render wrote filed no crop — the reason is the finding"*). So the *one query* this row prices is now a log read. **But there is still no line on `slots.length === 0`** — that branch guards the mint and logs nothing — which is the outcome the row is actually about, so the claim holds as written. ✅ **CLOSED 2026-09-12 (#64 item 2, foreman-20260912-1215).** `refineService.ts` now logs `[refineService] this render filed nothing in the library — no slot was minted` at `info` on `slots.length === 0`, carrying the ingredients the mint was handed (`earned`, `disputed`, `held`, `awaitingCarrier`, `open`, `unfiled`, `unfiledOpen`, `readerUnavailable`) so the reason is read off the line rather than guessed. Driven in `refineService.test.ts` — red with the branch removed, and a control render that DID file a slot never says it. | A decision to spend them. |
| **The `statedAccessories` read-back returns nothing on most renders that write it** | **THE REAL DEFECT under the withdrawn row, found 2026-08-11.** `captionRealization` returned null for `statedAccessories` on **2 of the 3** hoop renders in the capped batch (v#145, v#146) while `hairWorn` came back on every one. This is not observability — it is a reliability defect that silently costs a paid render its entire library contribution. | Its own chunk: drive the captioner on those two stored frames and find out whether it is the reader, the prompt, or the frame. Costs no credits — the frames are stored. ✅ **ANSWERED AND FIXED, UNDER ANOTHER NAME (fable-1364/1365, the founder's horns).** It was **the reader**. `mintedSlots.test.ts` carries the case as *"⚠ FILES THE ASK'S WORDS when the caption reader could not corroborate"*: v211 delivered horns, the BINDING verifier read them on the delivered frame and confirmed, and `captionRealization` then returned null because its own reader described them differently (*"pairs of curved bone-white horns"* against *"two clusters of three tapered points near each temple"*) — *"two honest readings of one thing, cancelled into deletion: no caption, no slot, no library row"*, so v212's prompt never said the word horns and the frame came back bare. `mintedSlotsForRender` now falls back to the ask's own words for a facet the binding verifier confirmed. The row is kept because its diagnosis (*a reliability defect that silently costs a paid render its entire library contribution*) was exactly right and is what the fix was aimed at. |
...
| **`stopline --prove` prints no total, so a park counts its controls by eye** | **RULED, NOT OPEN (fable-255).** This is how the instrument's own figure drifted: `12232246` took it from 10 to 14 controls, the next message wrote "15 controls" within the minute, and every park copied it forward for a day before opus-197 re-counted. Nothing touches the freeze instrument while the freeze holds; `npx tsx scripts/lib/stopline.mts --prove \| grep -c "^PASS"` is the driven count meanwhile. | A summary line (`14 of 14 controls passed`) riding any ordinary commit **after the walk**, with its own control — two lines, already priced. ⚠ **THE CLAIM HOLDS AND ITS BLOCKER IS SPENT (read 2026-08-24).** `scripts/lib/stopline.mts --prove` still ends on a sentence and no total, so the row is true. But the condition that parked the fix — *nothing touches the freeze instrument while the freeze holds* — no longer applies: there is no `.agents/mailbox/STOPLINE` file, the freeze is not holding, and the walk is long past. The two lines have been takeable for some time and nobody has taken them. ✅ **TAKEN 2026-09-12 (#64 item 1, foreman-20260912-1215).** `--prove` ends on `N of M controls passed` (15 of 15 at the tree), counted by the same `check` the PASS/FAIL lines come from; `spendingScriptArguments.test.ts` runs the prover as a child and holds the total to those lines — a counter that stops counting reddens it. |
...
## The stale-row reading this document was swept by (2026-08-24, opus-1175)

Ten candidate state claims read across §1 and §7's open-items table, **two
stale** — the `applied` gap (its cheap half landed) and the `statedAccessories`
read-back defect (answered and fixed under another name, and it was the reader)
— both repaired above in the additive form, the clause kept, the correction
beside it with its artifact.

**Two rows were read closely and are NOT stale, and saying so is the point of a
disposition table**: the zero-contribution log row (the `unfiled` vocabulary got
richer, but no line still announces `slots.length === 0`) and the `stopline
--prove` row (still no total — though its blocker is spent, which the row now
says). The bilateral-specimen row holds: `noSpecimen` is live throughout
`referenceCompleteness.ts`.

**Both of those two rows CLOSED on 2026-09-12** (#64 items 1 and 2, his own
ruling that they are one sitting): the total line is printed and held to its
own PASS/FAIL lines, and `slots.length === 0` announces itself with the mint's
ingredients. The rows above carry the receipts.
...
stale-claim word at any point. §7's table is a good surface for this sweep
precisely because its rows CARRY their state; the prose in §0 through §6 is not
retired by this reading.
