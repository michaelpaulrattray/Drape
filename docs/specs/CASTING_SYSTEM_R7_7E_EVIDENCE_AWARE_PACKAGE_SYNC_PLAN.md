...
For the first enabled recipe, accepting a left- or right-upper-chest tattoo
makes the existing `sideFull` / Walk selection stale. A centre-chest tattoo
does not stale Walk because a strict profile cannot reliably show the centre
anterior surface. When Walk is stale, the user sees one direct action:

```text
Update Walk · 300 credits
```

The result lands into the normal Walk slot. There is no second Accept
ceremony: Accept was required for the first canonical feature commit; a later
view synchronization is an explicit package operation whose strict probes are
the commit gate.
...
conditionally-visible generated sibling:
  sideFull for side left | right
  none for side centre
```
...
| `sideFull` | left/right: generate a Walk exposing that anatomical side and verify the observed side; centre with an existing current Walk: unaffected; centre with a missing Walk: generate a strict profile, omit the centre-chest mark, and probe both omission and framing |
...
`(canonical angle, ontology version, zone, surface, feature side)`. For every
visible tuple it also owns:
...
The generated image, not the prompt, is final truth. If the probe cannot
establish the actual visible anatomical side, or it observes the opposite
side, the output cannot commit.
...
For a left/right tattoo, the server chooses the Walk travel/facing instruction
that should expose that anatomical side. The prompt names anatomical
laterality explicitly; it never infers anatomy from frame-relative language.
Prompted direction is not accepted as truth: the strict probe must establish
what side the generated image actually shows.
...
For lateral Walk synchronization, the observed anatomical side must equal the
registry's required visible side. Requested direction, filename, canonical
slot name, and prompt text are never substituted for this image-derived proof.
...
- Current pilot Walk synchronization costs 300 credits.
...
- accepted panel changes its terminal copy from a dead-end warning to
  “Tattoo saved. Update the views that can show it.”
- a stale lateral Walk tile shows one `Update Walk · 300 credits` action;
- centre-chest acceptance leaves a current Walk alone and shows no Walk action;
...
- one deliberate Walk refresh and zero-generation mint ceremony;
...
- left/right Walk impact versus centre Walk compatibility;
- exact 300-credit server plan for an affected Walk;
...
- requested-direction drift cannot pass without observed-side proof;
...
- stale Walk success creates one asset + one package snapshot and carries the
  feature selection;
...
- accepted lateral Ink shows one obvious Walk action; centre Ink shows none;
- action displays 300 credits before click;
- no generation before click;
- Walk retains the same person and motion framing, exposes the correct
  tattoo-bearing anatomical side regardless of prompted-direction drift, and
  records the verified observed side in provenance;
- no invented mark;
- failure refunds and leaves old Walk selected;
- success lands once, survives reload/tab change, and removes stale state;
...
No new founder product decision is required to begin bounded implementation:

- explicit paid actions are already ratified;
- `slotCost` already fixes Walk at 300 credits;
- natural-language interaction and server-owned anatomy are D-70;
- Walk direction/semantics are D-39 as amended by D-71: direction is
  server-selected for evidence visibility and verified from the image;
