...
| **D7** `CHAR_OPTIONS` seven feature vocabularies | Deliberately dropped | The brief carries features as prose. This is the central architectural difference between V1 and V2 and is the plan's ratified position, not an omission. |
| **D8** eyebrow style expansions | Deliberately dropped | Expansions for enum values V2 does not have. A8's restored brow protocol covers rendering. |
| **D9–D13** hair families, sub-axes, legality matrix, buzz suppression, colour split | Deliberately dropped | All are form-vocabulary and form-legality machinery. V2's equivalent guarantee — that impossible combinations never reach the prompt — comes from the interpreter emitting a closed schema rather than from a legality matrix. |
