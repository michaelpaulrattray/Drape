...
| `mintModelAtomically` ⚠ | ~~`generation.mintPackage`~~ — **NO PRODUCTION IMPORTER as of 2026-08-17** (opus-604, ruled fable-815 §3): every textual use in `server/` and `client/`, dynamic destructuring included, is the definition, the barrel re-export, or a test. This row is **a specification, never a report** — it describes a CAS that no production code reaches through this symbol. | Model lock plus draft/status/owner/revision CAS in the `UPDATE` itself | Safe. Add `deletedAt IS NULL` after migration 0009 for explicit compatibility, while preserving the existing CAS. **Blocked on the prior question: where minting actually commits has not been read** — owed, owner in `POST_SIGN_ROADMAP.md` §7 (L8). |
