...
1. **Silhouette and composition distinguish node *types*.** A cast node (portrait image + chip strip), an image node (bare image), a note (text card), a frame (containment rectangle) read differently at any size because their shapes differ. Type is additionally in the external label at working zoom. We do not color-code node types.
...
- **Do not render blender chips on view nodes.**
...
Build order: tokens → `DottedGridBackground` → `TopBar` → `FloatingToolPill` → `ConnectionDot` → `NodeLabelRow` → `CanvasNodeShell` → `NodeInlinePrompt` → `NodeControlStrip` → `BlenderChipStrip` → `NodeFloatingToolbar` → `CastImageArea` → `NodeStatusBadge` → `CostLabel` → `CastNode` → popovers → studio. Each depends on the ones before it.
...
Prompt row at the card bottom: single-line input + right-aligned action button. States: `empty` (placeholder, Run disabled-ghost until input exists), `ready` (Run dark pill **with adjacent `CostLabel`**, §5.15), `generating` (input disabled, "Running" ghost), `complete` (button reads "Edit" → navigates to studio). `Enter` submits. Implementation as in the original draft, with one correction: in the `empty` state the Run button is **ghosted/disabled** (Run enables only when prompt text or a filled chip exists — foundations 3a); the original draft's "empty: Run dark and active, flashes red on press" contradicted the foundations and is dropped.
...
Chip strip below the control strip; **cast roots only**. Chip = popover trigger with three visual states — ghost (dashed hairline, `+ Brand`, faint), filled (solid hairline, `Brand · Editorial` + chevron), active (1px ink border, inset bg, medium). Popover content = the real tactile component (§13) in a `CanvasPopoverContent`. Implementation as in the original draft.

**The chips for a cast root, with real field names and formatters** (audit B2/B3/C/D/E/F — this table is normative; the original draft's `attrs.skin`/`attrs.brand`/`attrs.vibe` names were wrong):

```ts
// From useCastNodeController, when provenance.type === "cast_root":
const blenderChips = [
  { id: "brand",     label: "Brand",     value: attrs.castingBrand ?? null,                               popoverContent: <BrandSelectorPopover/> },   // width 260
  { id: "vibe",      label: "Vibe",      value: attrs.castingVibe ? formatVibe(attrs.castingVibe) : null, popoverContent: <VibeBlenderPopover/> },     // width 280
  { id: "ethnicity", label: "Ethnicity", value: formatEthnicity(attrs.ethnicityBlend ?? []),              popoverContent: <EthnicityBlenderPopover/> },// width 320
  { id: "skin",      label: "Skin",      value: attrs.skinTone?.split(" / ")[0] ?? null,                  popoverContent: <SkinTonePopover/> },        // width 280
  { id: "hair",      label: "Hair",      value: attrs.hairColor ?? null,                                  popoverContent: <HairColorPopover/> },       // width 300
  { id: "eyes",      label: "Eyes",      value: attrs.eyeColor ?? null,                                   popoverContent: <EyeColorPopover/> },        // width 240 — D-19
];
```

> **D-19 ratified with a flag:** six chips ship, but the chip row is explicitly re-evaluated at the first visual checkpoint (VC1's static strip preview) — if it feels crowded, dropping Eyes is a one-line change.
...
- Hair chip = `hairColor` only (audit D). Chip tooltips surface `*Override` text when present (parser override pattern).

When a chip's value was set by the parser rather than the user, it still renders filled — parser output and chip output are the same attribute surface; provenance of the value is not visually distinguished in pass 1.
...
- Below, when selected: `NodeControlStrip` (!isEmpty), `BlenderChipStrip` (root only — including empty roots, ghost state).
...
interface CastNodeControllerResult {
  ...
  blenderChips: BlenderChipProps[];   // [] for non-root
...
**Root empty/complete/view behavior** as in the original draft (empty root: placeholder art, focused prompt, ghost chips, disabled Run, no strip/toolbar; complete root: image, read-only prompt, Edit, filled chips, strip, toolbar; complete view: image, pose prompt, no chips, reduced toolbar) — with the pass-1 additions: costs on Run affordances, pinned exemptions, and the error state (§5.12).
...
- Selection drives: border weight (hairline → 1px ink), `NodeFloatingToolbar`, `NodeControlStrip`, `BlenderChipStrip` (roots), connected-edge opacity.

### Empty-state cast node (freshly dropped)
As foundations 3a: auto-selected, prompt auto-focused, five (six, pending D-19) ghost chips visible, no strip/toolbar, Run disabled until prompt text or a filled chip. First Run fires `boardOps.runGeneration` (server-side parser dispatch), with cost shown beforehand via `CostLabel`.
...
### Popovers
- One open at a time; opening another closes the first. Esc or outside-click closes and reverts the chip to its previous state.
...
| Control strip / chips | shown on selection | hidden | hidden |
