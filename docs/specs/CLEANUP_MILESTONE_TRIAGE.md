...
| **`shared/referenceIntents.ts` (a)** | *"the whole road is behind `CASTING_INK_STUDIO_SCOPE`, which is off"* | ⚠ **`off` is TRUE AGAIN as of 2026-09-24 — the studio retired (#1158 slice 4c) and the variable is unset; one line wrong in both directions inside six weeks.** As read then: it is `users:1`, read at `scripts/lib/productionFlagPositions.mts` — the declared table the deploy rite compares to the SERVICE on every push. |
