...
### D-79 — Partial deference: the unit of "said" is the fact, not the axis *(founder ruling 2026-08-01, four conditions; executor-implemented, advisor-reviewed)*

**The doctrine is unchanged at its root.** Null still means the brief did not
say. What changes is the granularity: a brief that speaks to one part of hair
no longer silences all of it.

**What forced it, measured rather than argued.** A paid sheet on "a skincare
founder in his 40s, silver at the temples" came back as eight men with the same
short dark greying hair. The word "silver" tripped hair deference, which
suppressed every authored hair line, so the image model chose one look and
repeated it — while the persisted identities claimed mid-length, cropped, short
and long. The record asserted difference the picture never showed. It was found
while verifying the twin-breaker (D-80's sibling work), and it was a larger
driver of sameness on that sheet than any pair rule.

**The four conditions, all binding:**

1. **The interpreter decomposes, not keyword matching.** It emits `hairSpoken`
   over a closed set — colour, cutLength, texture, coverage — and falls back to
   listing all four when it cannot tell, which collapses to the old behaviour.
   Fail toward safety, never toward guessing.
2. **Authored sub-axes are conditioned on the stated fragment.** Compatibility
   is the new contradiction test: silver temples restrict the authored base
   colour to shades silver reads against, never blonde. **Code owns what
   compatible means; the model only classifies.** A palette channel handed to a
   language model is the smuggling `CastingIntent` exists to prevent.
3. **Coverage stays all-or-nothing.** "Shaved head", "bald", "buzzed" suppress
   the whole axis — there is no cut on a bald man. Guarded twice: by the
   decomposition and by a word list that cannot be talked out of it.
4. **Stated words stay verbatim and senior.** Authored lines complement and
   never restate: no colour word when colour was claimed, no named cut when the
   cut was claimed, and no texture-bearing cut name when texture was claimed.

**Greying is an overlay, not a claim** — the one place the ruling is subtler
than a boolean. "Silver at the temples" says something is happening TO the hair
while the colour underneath is still open, so it sets `greyOverlay` and does not
claim `colour`. The resolver already modelled this shape ("grey is applied after
colour, not instead"); this is that idea reaching the brief.

**The keyword gate is one-way and stays.** Keywords may force MORE deference,
never less. The interpreter refines within a brief that mentions hair; it cannot
unlock one that does not, and it cannot claim hair was spoken when no hair word
exists. That asymmetry also converts a missing field — which naively reads as
"author everything" — into full deference, and it is the only signal the
deterministic fallback compiler has.

**Two defects this found in its own verification**, both fixed and pinned:
coverage had to be checked BEFORE the mention gate, because "a buzzed marine"
and "a balding accountant" contain no word in `HAIR_WORDS` and were being
declared hair-free; and the colour pair-breaker was shifting a compatible
chestnut to auburn, reintroducing the contradiction the resolver had just
avoided.

**Also closed:** suppressed sub-axes are now null in the persisted identity
rather than carrying a fabricated value no image rendered. That fabrication is
exactly what made the original report look like a resolver problem when the
resolver had been overruled.

**Not decided here, flagged for the record:** "shaven" remains in `HAIR_WORDS`,
so a clean-shaven brief still suppresses hair entirely. That over-deference
predates this change; loosening it is a separate, deliberate decision.
