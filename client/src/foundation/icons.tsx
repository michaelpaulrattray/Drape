 ...
 *
 * PROVENANCE — and which copy is the real one (#280)
 *
 * The founder drew this set and handed it over as
 * `docs/specs/Casting-ui-ux-design/drape-redesign/icons.tsx`, amended twice
 * (`P.settings`/`P.cog`, then the `cinema` glyph). **THIS file is the product's
 * copy and the only one that compiles; the handoff under `docs/` is the record
 * of what he sent and is never edited to match code.** A new glyph is added
 * here. If the two ever disagree, this one is what the app draws — say so in
 * the PR rather than quietly syncing them, because the handoff is evidence of
 * a decision and not a second implementation.
 *
