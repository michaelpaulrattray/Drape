 ...
 *
 * **It runs on `ModalScrim`, the shared shell's behaviour** (#262, his ruling:
 * *"Rename, delete-by-typing and sign are one shell with different contents"*).
 * It used to build its own `createPortal`, its own scrim and its own Escape
 * handler while borrowing the shell's classes — two owners of what a modal
 * DOES, and this was the copy with NO FOCUS TRAP: Tab walked straight out of
 * the field into the page behind the scrim. The lighter card is what stays
 * different; the behaviour underneath is not a second implementation.
 *
 * The containing-block warning now lives with the scrim, which is the thing
 * that portals.
 */
export function RenameDialog({
  ...
  useEffect(() => {
    ...
    */
    ...
    // Escape belongs to `ModalScrim` now — one owner, and it already refuses
    // while `busy`, which is the rule this copy also had.
  }, []);
