  ...
  return (
    <div className="w-full flex flex-col space-y-4 select-none">
      ...
      <div className="flex justify-center">
        <div className="bg-[#f5f3ef] border border-[rgba(0,0,0,0.05)] p-0.5 rounded-full flex relative">
          <button
            onClick={() => setActiveTab('Dyed')}
            className={`px-8 py-2 rounded-full text-[10px] font-medium tracking-wide transition-all duration-300 ${activeTab === 'Dyed' ? 'bg-[#1a1a1a] text-white shadow-sm' : 'text-[#999] hover:text-[#555]'}`}
          >
            Dyed
          </button>
          <button
            onClick={() => setActiveTab('Natural')}
            className={`px-8 py-2 rounded-full text-[10px] font-medium tracking-wide transition-all duration-300 ${activeTab === 'Natural' ? 'bg-[#1a1a1a] text-white shadow-sm' : 'text-[#999] hover:text-[#555]'}`}
          >
            Natural
          </button>
        </div>
      ...
      <div className="space-y-3 pt-2" style={{ borderTop: '1px solid rgba(0,0,0,0.05)' }}>
        <div className="flex items-center">
          <span style={{ fontSize: 9, fontWeight: 500, color: '#999' }}>
            Tone
          </span>
        </div>

        <div className="flex bg-[#f5f3ef] p-0.5 rounded border border-[rgba(0,0,0,0.05)]">
          {['Warm', 'Neutral', 'Cool'].map((t) => (
            <button
              key={t}
              onClick={() => setTone(t as 'Warm' | 'Neutral' | 'Cool')}
              className={`flex-1 py-1.5 rounded-sm text-[9px] font-medium tracking-wide transition-all ${tone === t ? 'bg-[#1a1a1a] text-white shadow-sm' : 'text-[#999] hover:text-[#555]'}`}
            >
              {t === 'Cool' ? 'Cool / Ash' : t}
            </button>
          ))}
