...
export type ExportStep = "idle" | "preparing" | "generating-pdf" | "compressing" | "done";
...
export function useExportPack({ modelId, assets }: UseExportPackParams) {
  ...
  const downloadZip = useCallback(async () => {
    ...
    setProgress(5);
    try {
      if (!planQuery.data) throw new Error("The casting package could not be confirmed. Close the dialog and try again.");
      ...
      const outcome = await withExportEligibility(
        ...
        async (_exportId, mutations) => {
          ...
          requireCompleteExportViews(prepared, planQuery.data.viewCount);

          // The PDF is mandatory. Nothing is archived or downloaded until it
          // exists, so "Identity Pack" can never mean an images-only fallback.
          setStep("generating-pdf");
          setProgress(72);
          const { lookIssues } = await deliverAtomicIdentityPack({
            generatePdf: () => mutations.generatePdf({ modelId, images: preparedPdfImages(prepared) }),
            buildArchive: async (pdf) => {
              const zip = new JSZip();
              for (const view of prepared) zip.file(view.filename!, dataUrlToBytes(view.dataUrl!));
              ...
              setStep("compressing");
              setProgress(90);
              return { content: await zip.generateAsync({ type: "blob" }), lookIssues };
            },
            deliver: ({ content }) => downloadBlob(
              content,
              `CASTING_PACK_${safeStem(modelName.toUpperCase())}_CURRENT.zip`,
            ),
          });

          if (lookIssues.length > 0) {
            toast.warning("Identity pack downloaded; some saved looks were omitted", {
              description: lookIssues.join(" "),
            });
          } else {
            toast.success("Identity pack downloaded");
          }
        },
      );
      if (!outcome.ok) notifyExportRefusal(outcome.reason);
    } catch (error) {
      toast.error("Identity pack was not downloaded", {
        description: `${exportFailureMessage(error)} Nothing was downloaded and no credits were charged.`,
      });
    } finally {
  ...
  }, [assets, generatePdfMutation.mutateAsync, model, modelId, modelName, notifyExportRefusal, planQuery.data, proxyImageMutation.mutateAsync, savedLooks]);
  ...
  return {
    ...
    exportPlanError: planQuery.isError ? "The casting package could not be confirmed." : null,
