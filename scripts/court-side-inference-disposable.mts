/**
 * DOES A SIDE READ OUT OF HER SENTENCE LAND ON HER SIDE? (fable-604 §3b.)
 *
...
const OUT = "output/side-inference-court";
...
const ARMS: Array<{ arm: string; side: "left" | "right" | "both"; ask: (colour: string) => string }> = [
  ...
  { arm: "both", side: "both", ask: (colour) => `her left and right eyes ${colour}` },
];
...
for (const arm of ARMS) {
  for (const colour of arm.side === "both" ? COLOURS.slice(0, 2) : COLOURS) {
    /* Back to the parent, so every render is the same comparison. */
    await conn.execute(
...
const both = results.filter((row) => row.arm === "both" && Number.isFinite(row.leftChange as number));
const evenness = both.map((row) => {
  const left = Number(row.leftChange);
  const right = Number(row.rightChange);
  return Math.max(left, right) / Math.min(left, right);
});
console.log(`  CONTROL both: ${evenness.filter((value) => value <= BOTH_BAR).length}/${both.length}`
  + ` renders moved BOTH eyes within ${BOTH_BAR}x — ${evenness.map((value) => value.toFixed(2)).join(" · ")}`);
