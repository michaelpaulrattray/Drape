/** Guarded disposable-MySQL gate for the R7-5C atomic deletion service. */
...
const PREFIX = "drape_r7_5c_disposable_";
...
async function main() {
  ...
  try {
    ...
    const test = await openDatabase({ uri: testUrl.toString(), connectTimeout: 15_000 });
    try {
    ...
    run(process.platform === "win32" ? "pnpm.cmd" : "pnpm", [
      ...
      R2_PUBLIC_URL: "https://pub-r7-5c-test.r2.dev",
    });
    console.log("[disposable] R7-5C atomic deletion, rollback and writer-fence gates passed");
  } finally {
    if (created) {
