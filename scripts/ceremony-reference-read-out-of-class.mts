/**
 * Ceremony — the out-of-class outcome (`casting_reference_reads.outcome`,
 * migration 0042; ordered fable-1068 §4).
 *
 * The reader gained a fifth way to end: it can now answer *this is not
 * cosmetics* rather than describing a prosthetic as a look. That refusal needs
 * a value on the column's enum, because MySQL's own answer to a value outside
 * an enum is to write the EMPTY STRING — a row that counts toward the tally and
 * names nothing.
 *
 ...
 * tally is the only record that the new refusal is firing at all.
 *
 ...
 *
 *   npx tsx scripts/ceremony-reference-read-out-of-class.mts --dev
 *     reads DATABASE_URL from .env — the dev database, and only that
 *
 *   railway.cmd run --service MySQL -- npx tsx scripts/ceremony-reference-read-out-of-class.mts --production
...
try {
  ...
  */
  ...
  if (before.includes(VALUE)) {
    console.log(`ALREADY APPLIED — \`${VALUE}\` is on the column.`);
  } else {
    ...
    const sql = await readFile("drizzle/0042_reference_read_out_of_class.sql", "utf8");
    for (const statement of sql.split("--> statement-breakpoint")) {
      const trimmed = statement.trim();
      if (trimmed) await conn.query(trimmed);
    }
    const after = await readOutcomeValues(conn);
    if (!after.includes(VALUE)) throw new Error("the migration ran and the value is not on the column — stop and investigate");
