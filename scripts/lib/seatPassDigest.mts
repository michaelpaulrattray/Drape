 ...
 * comments say what happened. So every outcome here is the CARD BUILD BOARD's
 * phrase — `scripts/lib/cardBuildState.mts`'s `buildBoard`, the one reader every
 * queue reader consults (#1094) — over a board read AFTER the seats have
 * finished. The relay and his page therefore read the same words about the same
 * pull request. A card with no artifact of any kind reads *"nothing recorded"*,
 * which is the honest sentence and the one a relay should go and look at.
 *
 * Pure: readings in, markdown out. `server/seatPassDigest.test.ts` drives it.
 */
import type { SeatBuildBoard } from "./seatBatches.mts";
...
/** One card's outcome, in the board's own words. */
export function passCardOutcome(board: SeatBuildBoard, card: number): string {
  return board.phraseFor(card) ?? "nothing recorded — no PR, no claim, no refusal";
}
...
export function renderPassDigest(input: PassDigestInput): string {
  ...
  if (input.handout.length === 0) {
    ...
    for (const card of [...input.handout].sort((a, b) => a.seat - b.seat || a.number - b.number)) {
      out.push(line(card, passCardOutcome(input.board, card.number)));
    }
  }
  out.push("");
