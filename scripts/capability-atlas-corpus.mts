...
export const CORPUS: readonly CorpusRow[] = [
  ...
  { id: "ink.transform.wrongslot", ask: "his upper chest tattoo — make it bigger", subject: "ink", verb: "change", state: "branch-with-ink",
    expect: "free:noInkToChange", why: "⚠ OPEN QUESTION (fable full-map pass): a CHEST-worded transform on a branch wearing only an ARM tattoo reached the claim in drive-5 — which tattoo would it change? The belief is the place-speaking free answer; the mismatch stands until the executor reads the route" },
  ...
  { id: "hair.remove.none", ask: "remove her fringe", subject: "hairStyle", verb: "remove", state: "master",
    expect: "would-render", why: "MODEL-VARIANT: measured would-render twice and removal_absent once across three drives — the fringe reads as a cut change (law 8) or a removal by the model's coin; the mismatch, when it appears, is the variance and not a route change" },
