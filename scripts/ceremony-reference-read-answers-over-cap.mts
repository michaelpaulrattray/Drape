/**
 * Ceremony — the read we could not use (`casting_reference_reads.outcome`,
 * migration 0065; issue #1076).
 *
 * The makeup reader is gaining a tenth ending on this column. It can now say
 * *we looked, we answered, and not one of our answers was short enough to use*
 * — which until now wore `no_makeup_visible`, a sentence about HER photograph
 * for a fault that is entirely ours.
 *
 * WHY THIS IS A CEREMONY AND NOT THE RITE'S JOB. `outcome` is a MySQL enum and
 * widening one is a `MODIFY COLUMN`, which `scripts/lib/ceremonyAutoApply.mts`
 * refuses by design rather than guessing: telling a widening from a narrowing
 * needs the current members compared with the target's, and a classifier
 * sometimes right about `MODIFY` is worse than one never asked. The refusal
 * costs exactly this one command, and it names the statement so the command is
 * never a guess.
 *
 * Running behind this ceremony costs the TALLY and never a customer's answer:
 * `recordReferenceRead` proves the outcome against the column's own list before
 * the insert and logs instead of writing. It is still applied first, because
 * the tally is the only record that the new refusal is firing at all.
 *
 ...
 *
 *   npx tsx scripts/ceremony-reference-read-answers-over-cap.mts --dev
 *     reads DATABASE_URL from .env — the dev database, and only that
 *
 *   railway.cmd run --service MySQL -- npx tsx scripts/ceremony-reference-read-answers-over-cap.mts --production
...
/**
 * The migration this ceremony carries, with the value that proves it.
 *
 * The file is REPLAYED rather than retyped — a ceremony that re-types its own
 * DDL is a second copy of the schema and it drifts from the one every test ran
 * against (working law 4).
 */
const MIGRATION = {
  file: "drizzle/0065_reference_read_answers_over_cap.sql",
  values: ["answers_over_cap"],
  what: "the read we could not use, counted as ours",
} as const;
...
try {
  ...
  */
  ...
  for (const control of ["delivered", "unreadable", "no_makeup_visible"]) {
  ...
  if (MIGRATION.values.every((value) => before.includes(value))) {
    ...
    /* Each value named in its own failure: an alter that landed one of two is a
       half-applied migration, and a check for "any of them" reports success on
       it. One value today, and the shape survives the next one. */
