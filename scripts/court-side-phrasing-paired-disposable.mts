/**
 * THE PAIRED PASS — the same colour asked both ways, so the clause is the only
 * difference. (Follows the unpaired court, which read 3/6 plain and 6/6 placed
 * on her right and is suggestive rather than conclusive at n=6.)
 *
...
const OUT = "output/side-phrasing-paired";
...
const COLOURS = ["crimson", "silver grey", "turquoise", "copper", "indigo", "olive green"];
const ARMS: Array<{ arm: string; side: "left" | "right" | "both"; ask: (colour: string) => string }> = [
  { arm: "plain", side: "right", ask: (colour) => `her right eye ${colour}` },
  { arm: "placed", side: "right", ask: (colour) => `her right eye ${colour}` },
];
/** SIX FRESH COLOURS, EACH ASKED BOTH WAYS. The first court compared six
 *  phrased renders against six unphrased ones run an hour apart; this one runs
 *  the same colour through both wordings on the same parent, so the only thing
 *  that differs between a pair is the clause. */
const REPEATS: Record<string, number> = { plain: 6, placed: 6 };
...
/* THE THING UNDER TEST — set per render, below. */
...
for (const armName of ["plain", "placed"] as const) {
  ...
  const wrongEye = ratios.filter((ratio) => ratio < 1).length;
  console.log(`  ${armName.padEnd(7)}: ${passed}/${arm.length} over ${RATIO_BAR}x`
    + ` · WRONG EYE ${wrongEye}/${arm.length}`
    + ` — ratios ${ratios.map((ratio) => ratio.toFixed(2)).join(" · ")}`);
}
console.log("");
console.log("the first, unpaired pass: plain 3/6 correct eye · placed 6/6");
