/**
 * THE BEARD REMOVAL COURT — V3 slice (b)'s measuring stick (fable-531 §4a).
 *
...
const NEXT = "Change only this: his t-shirt is black.";
 ...
 */
const VACATE = ` ${VACANCY_BY_KIND["facial hair"]!.says}.`;
...
async function judge(bytes: Buffer, label: string): Promise<{ beard: boolean; clean: boolean; saw: string }> {
  const answer = await reader.complete({
    ...
    user: "Answer as {\"facial_hair\": true|false, \"artefact\": true|false, \"saw\": \"<a few words>\"}. "
      + "facial_hair: does this person have a beard, moustache or visible stubble? "
      + "artefact: is there anything odd on the jaw, chin or upper lip — a shadow, a "
      + "discoloured patch, a colour break against the cheek, a smudge or a blurred edge?",
  ...
  const beard = parsed?.facial_hair === true && seen;
  const clean = parsed?.artefact === false && seen;
  say(`    judge ${label.padEnd(20)} beard=${beard ? "YES" : "no "} clean=${clean ? "YES" : "no "}  ${saw.slice(0, 62)}`);
...
say("SPECIMEN — growing the beard first, so the removal has something to remove");
const master = await render(
  candidate,
  "Change only this: he has a full, thick dark beard and moustache, neatly grown.",
  "master",
);
const grown = await judge(master, "master");
if (!grown.beard) {
  say("");
  say("THE SPECIMEN FAILED: the beard did not grow, so nothing below could measure a removal.");
...
say("");
say(`CONTROL ARM — ${N} renders, the beard never mentioned`);
let controlKept = 0;
...
for (const frame of shaved) {
  const after = await render(
    frame.bytes,
    `Change only this: he is wearing a grey scarf.${VACATE}`,
...
say("");
say("=".repeat(78));
say(`CONTROL   beard still there   ${controlKept} of ${N}   (the bar is ${N} — the judge can SEE a beard)`);
say(`REMOVAL   gone                ${gone} of ${N}`);
say(`          skin clean          ${clean} of ${N}`);
say(`SURVIVAL  still gone          ${held} of ${N}   (the arm born-worn removals fail)`);
const verdict = controlKept === N && gone === N && clean === N && held === N
  ? "PASS — a beard in the master can be removed, cleanly, and stays removed"
  : "SHORT — see the arms above; nothing is promoted on this reading";
say(verdict);
say("=".repeat(78));
...
const sheet = await buildContactSheet({
  columns: ["bearded master", "control (no mention)", "removal", "survival"],
