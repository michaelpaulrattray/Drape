...
/*
  ⚠ THE SAME BOARD THE CUT READ, re-read AFTER the seats have finished, through
  the one owner of that read (#1094). A half that could not be read is NAMED in
  the digest rather than printed as silence: here, unlike in the cut, a partial
  board is not a reason to refuse — the pass is already over and this file is its
  only record, so the honest thing is to write it and say what was missing.
*/
...
/* The labels the verdict count reads live on the pull-request rows themselves,
   so they come off the same list the board was built from rather than a second
   `gh` call asking a different question at a different moment. */
