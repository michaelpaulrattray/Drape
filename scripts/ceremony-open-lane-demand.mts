/**
 * Ceremony — the open lane's demand record (`casting_open_lane_demand`,
 * migration 0031).
 *
 ...
 *
 *   npx tsx scripts/ceremony-open-lane-demand.mts --dev
 *     reads DATABASE_URL from .env — the dev database, and only that
 *
 *   railway.cmd run --service MySQL -- npx tsx scripts/ceremony-open-lane-demand.mts --production
...
const TABLE = "casting_open_lane_demand";
...
try {
  ...
  */
  ...
  if (before.length === 1) {
    console.log("ALREADY APPLIED — the demand table is here.");
  } else {
    ...
    const sql = await readFile("drizzle/0031_casting_open_lane_demand.sql", "utf8");
    ...
    console.log("APPLIED — the demand table exists.");
  }
  ...
  */
  ...
  for (const forbidden of ["userId", "candidateId", "instruction", "storageKey", "requestText"]) {
