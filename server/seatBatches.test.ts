...
const NO_PRS = [] as const;
const NOW = Date.parse("2026-09-26T12:00:00Z");

function population(cards: SeatCandidateCard[], switches = ALL_ON) {
  return seatPopulation({
    cards,
    switches,
    openPullRequests: NO_PRS,
    facts: NO_FACTS,
    areaIndex: INDEX,
    nowMs: NOW,
  });
...
describe("what a background seat may take", () => {
  ...
  it("never takes a card with an open pull request", () => {
    const { takeable, skipped } = seatPopulation({
      cards: [card(17, ["bug"])],
      switches: ALL_ON,
      openPullRequests: [{ number: 900, title: "something (#17)", body: null, headRefName: null }],
      facts: NO_FACTS,
      areaIndex: INDEX,
      nowMs: NOW,
    });
  ...
  it("never takes a card claimed in the last twelve hours, and takes one claimed before that", () => {
    const live = seatPopulation({
      cards: [card(18, ["bug"])],
      switches: ALL_ON,
      openPullRequests: NO_PRS,
      facts: [{ kind: "claim", card: 18, seat: "seat-2", at: "2026-09-26T09:00:00Z" }],
      areaIndex: INDEX,
      nowMs: NOW,
    });
    ...
    const stale = seatPopulation({
      cards: [card(18, ["bug"])],
      switches: ALL_ON,
      openPullRequests: NO_PRS,
      facts: [{ kind: "claim", card: 18, seat: "seat-2", at: "2026-09-24T09:00:00Z" }],
      areaIndex: INDEX,
      nowMs: NOW,
    });
  ...
  it("never takes a card a seat has refused", () => {
    const { takeable } = seatPopulation({
      cards: [card(19, ["bug"])],
      switches: ALL_ON,
      openPullRequests: NO_PRS,
      facts: [{ kind: "refusal", card: 19, at: "2026-09-26T11:00:00Z" }],
      areaIndex: INDEX,
      nowMs: NOW,
    });
    expect(takeable).toEqual([]);
