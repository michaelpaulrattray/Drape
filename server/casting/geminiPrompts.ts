...
// ============================================================================
// DEPRECATED ALIASES — Remove after Phase 1b/1c updates callers
// ============================================================================

/**
 * @deprecated Use BRAND_PROFILES[brand].descriptor instead.
 * Kept for backward compatibility until geminiGeneration.ts is updated.
 */
export const getBrandDescriptors = (brand: string): string => {
  const profile = BRAND_PROFILES[brand];
  if (profile) {
    // Extract just the adjective portion (before EXPRESSION:)
    const desc = profile.descriptor.split('EXPRESSION:')[0].trim();
    // Remove brand name prefix (e.g. "Gucci — ")
    return desc.replace(/^[^—]+—\s*/, '');
  }
  return DEFAULT_BRAND_DESCRIPTOR;
};

/**
 * @deprecated Use getBrandExpression(brand) instead.
 * Kept for backward compatibility until geminiGeneration.ts is updated.
 */
...
/**
 * @deprecated Replaced by inline technicalConstraints in geminiGeneration.ts.
 * Kept for backward compatibility until geminiGeneration.ts is updated.
 */
