...
export interface InkProjectionCancelledResult {
  candidateId: string;
  status: "cancelled";
...
export async function commitCancelInkAddIntent(input: {
  ...
  return withTransaction(async (tx) => {
    ...
    if (!intent || (!legacyIntent && !anywhereIntent)) {
...
export async function commitCancelInkProjectionCandidate(input: {
  userId: number;
  modelId: number;
  candidateId: string;
  operationId: string;
  ...
  return withTransaction(async (tx) => {
    ...
    const result: InkProjectionCancelledResult = {
