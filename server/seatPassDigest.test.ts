...
const base = {
  ...
  openPullRequests: [
    { number: 1350, title: "fix the roll (#300)", body: null, headRefName: "team/roll-300", labels: [] },
    { number: 1351, title: "money", body: "for card #400", headRefName: null, labels: ["needs-fable"] },
  ],
  facts: [{ kind: "claim" as const, card: 301, seat: "seat-1-20260926", at: "2026-09-26T11:30:00Z" }],
...
describe("a card's outcome comes from the artifacts", () => {
  it("names the pull request that builds it", () => {
    expect(passCardOutcome({ card: 300, openPullRequests: base.openPullRequests, facts: [], nowMs: NOW }))
      .toContain("PR #1350");
  });

  it("names who claimed it when there is no pull request", () => {
    expect(passCardOutcome({ card: 301, openPullRequests: base.openPullRequests, facts: base.facts, nowMs: NOW }))
      .toContain("claimed by seat-1-20260926");
  });

  it("says plainly that nothing was recorded rather than implying a failure", () => {
    const line = passCardOutcome({ card: 999, openPullRequests: base.openPullRequests, facts: [], nowMs: NOW });
  ...
  it("does NOT read a pull request that merely cites the card", () => {
    ...
    const cited = passCardOutcome({
      card: 493,
      openPullRequests: [{ number: 1360, title: "something", body: "the law from #493 applies", headRefName: "team/x-77" }],
      facts: [],
      nowMs: NOW,
    });
