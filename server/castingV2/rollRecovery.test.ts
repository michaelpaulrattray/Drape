  ...
  getDb: async () => ({
    select: () => ({
      from: (table: unknown) => ({
        where: (..._args: unknown[]) => {
          ...
          const source = name === "casting_rolls" ? rows.rolls : rows.candidates;
    ...
    update: (table: unknown) => ({
      set: (values: Record<string, unknown>) => ({
        where: async () => {
          const name = getTableName(table as never);
          const target = name === "casting_rolls" ? rows.rolls : rows.candidates;
...
const OPERATION = { id: OPERATION_ID, userId: 7, chargedCredits: 160, refundedCredits: 0 };
...
beforeEach(() => {
  rows.rolls = [{ id: 1, operationId: OPERATION_ID, userId: 7, status: "generating" }];
  rows.candidates = [];
  refunds.length = 0;
  refundRecords = true;
  vi.clearAllMocks();
...
describe("crash: after the claim, before any rows were committed", () => {
  it("refunds nothing, because the charge happens only after rows are durable", async () => {
    rows.rolls = [];
    const outcome = await recoverCastingV2RollOperation(OPERATION);
    expect(outcome).toEqual({ type: "paid_failure", refunded: 0, refundedCredits: 0 });
...
describe("crash: every candidate landed", () => {
  it("finalises the roll complete and refunds nothing", async () => {
    ...
    const outcome = await recoverCastingV2RollOperation(OPERATION);
    expect(outcome).toEqual({ type: "durable_success", ready: 2 });
    expect(refunds).toHaveLength(0);
...
describe("the provider probe changes accounting, never the refund", () => {
  ...
  it("marks unrecovered when the provider says it never delivered", async () => {
    ...
    await recoverCastingV2RollOperation(OPERATION, { probe: async () => "not_delivered" });
  ...
  it("never probes a candidate that was never dispatched", async () => {
    ...
    await recoverCastingV2RollOperation(OPERATION, { probe });
