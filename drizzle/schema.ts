 ...
 *
 * DECLARED in `shared/auditActions.ts` and re-exported here (#940) so every
 * existing server importer keeps working unchanged. It moved because the
 * audit panels now DERIVE their category chip from the server bucket list
 * (`shared/auditActionCategories.ts`), and the client cannot import this file
 * — importing it would pull `drizzle-orm` into the browser bundle.
 */
export { AUDIT_ACTIONS } from "../shared/auditActions";
export type { AuditAction } from "../shared/auditActions";
...
export const castingSegments = mysqlTable("casting_segments", {
  ...
  */
  /*
    Its counterpart WAS `casting_cast_segments` — the promoted set a signed
    Cast was to keep, declared in 0027 ahead of a Sign promotion that was never
    written, and DROPPED in 0067 on his word (#1184, 2026-09-25) with zero rows
    in both worlds. The reasoning that made it a second table rather than a
    nullable column here still governs this key: it is keyed on the candidate,
    and a shared key would have to admit NULLs, which MySQL lets repeat.
  */
...
export const castingReferenceLibrary = mysqlTable("casting_reference_library", {
  ...
  /*
    ...
    not a key at all — the same reasoning that once made `casting_cast_segments`
    a second table rather than a nullable column on `casting_segments` (that
    table is gone, 0067; the reasoning is not).
  */
