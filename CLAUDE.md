...
⚠ **AND YOU NO LONGER RUN `pnpm architecture:generate` BY HAND — THE COMMIT DOES IT (#501, 2026-09-04), because remembering was measurably not working.** Of the last 60 `gate.yml` runs, **5 failed, and all 5 failed on the single step `Architecture Atlas freshness`** — each one a red at ~3½ minutes, a local regenerate, a re-push and a second seven-minute gate, for a change that was otherwise fine. `.githooks/atlas-stage` (ARM 3 of `pre-commit`) regenerates **both** maps and stages them whenever a commit touches a path they are built from, so a branch pushes the map of its own tree; a generator that fails REFUSES the commit rather than letting a map that could not be built ship as fresh. It costs ~9s (measured: architecture 7.6s + capability 1.6s) and **only on those commits** — a docs-only or workflow-only commit pays nothing. It needs no new per-clone config: `core.hooksPath=.githooks` already arms it, and the deploy rite already refuses to run without that. ⚠ **ONE CASE STILL NEEDS YOUR EYE AND THE HOOK SAYS SO OUT LOUD: A PARTIAL STAGE.** The generators hash the bytes **on disk**, not the index, so committing `server/foo.ts` while `server/bar.ts` carries unstaged work ships a map built from a tree that is not the one committed — and the gate, which rebuilds from the COMMITTED tree, reddens exactly as before. This is not new (a hand-run generator read the same disk) and it is not refused, because committing part of a working tree is a legitimate thing to do; the hook **names the offending files and tells you the map may not match**, instead of printing that the commit carries the map of its own tree. Stage them or stash them if you want the freshness check to agree. ⚠ **AND "A BRANCH PUSHES THE MAP OF ITS OWN TREE" WAS TRUE OF EVERY ROAD BUT THREE — CLOSED 2026-09-07 (#606, #519).** The sentence above rests on `pre-commit` running, and **git does not run it on every road to a commit.** Measured, one scratch repository per road on git 2.55: `git revert`, `git cherry-pick` and a **replayed rebase commit** all skip `pre-commit` (`--amend` does not — it runs it, so the amend road was never exposed). Under deploy-on-merge **every rollback proof is a revert**, so this is a road shifts now walk on purpose, and it bit on PR #605 exactly as #501 describes: a red at 3½ minutes, a regenerate, a second commit, a second seven-minute gate. ⚠ **The tempting repair does not work and was driven rather than assumed** — `prepare-commit-msg` fires on all three roads, but a hook that writes and stages a file there lands it in a cherry-pick and a rebase and **NOT in a revert**, where it fires twice and the staged file is dropped; a repair covering two of three roads and silently failing on the common one is worse than none, because it reads as coverage. So the answer is a GATE, not a repair, and it is one `main` already had: `merge-atlas`'s own stated limit ended *"the rite's freshness check is the backstop either way"* — true of `main`, which only the rite pushes, and **false of every shift branch, which never meets the rite.** `.githooks/pre-push` ARM 2 now runs the rite's own two checks before any branch push. ⚠ **It refuses only when its verdict is trustworthy**: both checks read the bytes on disk, so with uncommitted work in a path the generators read it WARNS and lets the push through rather than blocking a legitimate push on a question it cannot answer. `server/atlasPushGate.test.ts` drives it against real repositories, a real bare remote and a real `git push`, and every allow-arm asserts the remote actually MOVED — the first driver written for it had three arms reading `Everything up-to-date`, where nothing was pushed and the hook's verdict was never consulted.

⚠ **The path filter is a mirror of the generator's own `SCANNED_ROOTS` and mirrors drift (working law 4), so it is not left to hold** — `server/atlasCommitHook.test.ts` reads that constant out of `scripts/generate-architecture.mts` and reddens if a root the generator scans is not covered, and the same suite drives the hook against real temporary repositories in both directions. **The filter itself now lives in `.githooks/atlas-paths`**, sourced by `atlas-stage` and `pre-push` alike, because the moment it had two consumers a copy in each was the drift this repository has been bitten by before; neither hook may declare its own, and an arm holds both to sourcing it. ⚠ **`docs/architecture/annotations.yaml` is on the filter and #501's own body would have dropped it**: the card said "skip when only `docs/` is staged", and that file lives under `docs/` and is hashed into the architecture fingerprint — editing it alone is precisely the commit that goes stale.

⚠ **AND THE MAP NO LONGER CARRIES A SOURCE FINGERPRINT AT ALL — #1307, 2026-09-26, and it is the one line that made every concurrent PR conflict.** `meta.sourceFingerprint` hashed every scanned file, so **every branch moved it**: two PRs open at once could never agree on it, the moment either merged the other read CONFLICTING on GitHub, and **a conflicting PR fires no workflow run at all** (the #566 absent state) — so its gate went silent too. Measured with git's own three-way merge over the real blobs of the sixteen seat branches of that day: **120 of 120 pairs conflicted on `drape-architecture.json`, and with the line removed from all three sides, 0 of 120 did** — 44 of those pairs still both changed the map, and their real content merged clean. **Nothing had ever read the value to decide anything**: the freshness check regenerates the whole document and compares CONTENT, which is why it needed no replacement. The hash now goes to **`docs/architecture/source-fingerprint.txt`, gitignored** — a local answer to *"which tree is my `index.html` built from"* — and `meta`'s `additionalProperties: false` turns a re-added one into a schema error rather than a tax. `fingerprint()` itself stays, because `server/atlasCommitHook.test.ts` derives the hooks' warn population from the paths its body reads. ⚠ **What replaces the conflict, in the rare case, is a freshness RED and not a silent wrong map**: where two independent map edits interact — a count on one line that both sides move — git either conflicts (the `merge=atlas` driver, then a regeneration on the merged tree) or merges to something `pnpm architecture:check` calls stale. Both arms are driven on the real committed map in `server/atlasMergeDriver.test.ts`, with git configured the way GitHub has it: no driver, no hooks.
...
`sdk.ts` handles session JWT sign/verify only (no external OAuth server). `verifySession` requires a non-empty `appId` in the JWT payload, which is why `VITE_APP_ID` must be set. A session whose user is missing from the DB is rejected outright. (Owner notifications used to route through `_core/notification.ts` to Slack webhooks production never had — deleted with the Slack retirement, #800, 2026-09-11; the admin panel's audit log and overview alerts feed are the notification surface.)
...
| `CASTING_SEGMENTS_SCOPE` | the segment store (segment permanence) — ✅ RETIRED (#1160); the variable is off the service |
| `CASTING_SEGMENTS_DELIVERED_SCOPE` | cutting a segment from the delivered frame's own extent rather than the master's — ✅ RETIRED with its parent (#1160) |
...
- `SENTRY_DSN` — the server error tracker (#509 part 1). **Absent means there is no tracker at all**: `@sentry/node` is never imported (593 ms, measured) and the boot line says errors are reported nowhere rather than implying one is watching. What may leave the building is `shared/errorEventScrub.ts`'s allowlist, and a `masterPrompt`, `brief`, `prompt`, `preferences`, `technicalSchema`, `personaLine`, `resultUrl`, `imageKey` or `passwordHash` key anywhere in an event **refuses the whole event** — the metadata-only boundary applies to a third party exactly as it applies to staff. `server/monitoring/errorTracker.ts` enumerates the envelope channels that do NOT pass that gate and closes or declares each one
...
Production deploys when `main` moves — a squash-merged PR as much as the rite's
push — and the founder dogfoods paid rolls while that happens. A deploy that
lands mid-roll kills the process holding its candidates.

⚠ **THAT SENTENCE HAS BEEN TRUE, THEN FALSE, THEN TRUE AGAIN, AND THE MIDDLE
STATE COST A SHIFT A LIE (#296 → #508).** Until 2026-09-06 Railway watched
**`local-migration`**, which only the deploy rite pushed (`main:local-migration`),
so a squash merge moved `main` and shipped nothing — measured by driving it: PR
#294 merged at `09:46Z` and twenty-five minutes later production was still
serving the previous build, **no deployment ever created for the merge commit**.
This file said *"Every push to `main` deploys"* the whole time, and was corrected
on the morning of 2026-09-06 (#296). **That same evening the founder flipped the
service to watch `main` (#508, deploy-on-merge: pre-deploy migrations, a
healthcheck-gated cutover, a secretless `deploy-verify` job on every push to
`main`), and PR-2 deleted `local-migration`.** So the sentence at the top is true
again, for a different reason than it was first written. ⚠ **What did not
change is the trap**: every reading a shift naturally reaches for agrees with
the merge — `gh pr view` says `MERGED`, `git log origin/main` carries the
commit, and `/api/health` returns `200` **from the OLD process** until the new
one takes traffic. That is why the rite prints an `UPTIME ANCHOR` beside the
`200`, why `/api/health` now carries the `build` sha, and why the
`deploy-verify` job waits for THAT sha rather than a 200: an old process cannot
pass as a new one silently. The production URL a health check must actually use
is `https://drape-production-0232.up.railway.app` (`deploy-rite.mts`, and the
...
sentence and its siblings honest, deriving the deploying ref from
`DEPLOY_SOURCE_REF` rather than restating it — the guard's job inverted with
the flip: what no document may now say is that `local-migration` deploys, or
deny that a merge ships.
