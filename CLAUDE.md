...
Most of these followed the same path: helper or rule written, docs written, todo ticked, call site never added — and the last is the nastier variant, invoked but inert under the current configuration. **There is a third path, and the velocity pair below is the one that walked it: written, wired, live — and then orphaned by a deletion aimed at something else entirely.** It is the hardest of the three to catch, because at no point did anyone skip a step. Invariants 7 and 8 exist because of them. The grid above was re-verified cell-by-cell against the code on 2026-07-25.
